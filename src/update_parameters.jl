...
function update_parameter_in_affine_constraints!(
                            ...
                            affine_constraint_cache_inner::DD.WithType{F, S}) where {OT, T, F, S}
    for (ci, param_array) in affine_constraint_cache_inner
        update_parameter_in_affine_constraints!(optimizer, ci, param_array, parameters, updated_parameters)
    end
    return optimizer
end
...
function update_parameter_in_affine_constraints!(
    ...
    for term in param_array
        if haskey(updated_parameters, term.variable_index) # TODO This haskey can be slow
            param_constant += term.coefficient * (updated_parameters[term.variable_index] - parameters[term.variable_index])
        end
    end
    if param_constant != 0
...
function update_parameter_in_conic_constraints_soc!(model::ParametricOptimizer)
    conic_constraint_cache_soc_inner = model.conic_constraint_cache_soc[MOI.VectorAffineFunction{Float64}, MOI.SecondOrderCone]

    if !isempty(conic_constraint_cache_soc_inner)
        update_parameter_in_conic_constraints_soc!(model.optimizer, model.parameters, model.updated_parameters, conic_constraint_cache_soc_inner)
    end

    return model
end

function update_parameter_in_conic_constraints_soc!(
    optimizer::OT, 
    parameters::Dict{MOI.VariableIndex, T}, 
    updated_parameters::Dict{MOI.VariableIndex, T}, 
    conic_constraint_cache_nn_inner::DD.WithType{F, S}) where {OT, T, F, S}
    ...
    for (ci, param_array) in conic_constraint_cache_nn_inner
        update_parameter_in_conic_constraints_soc!(optimizer, ci, param_array, parameters, updated_parameters)
    end
...
function update_parameters!(model::ParametricOptimizer)
    ...
    update_parameter_in_conic_constraints_soc!(model)
