...
function solve_poi_no_params_glpk(data::PMedianData; vector_version, time_limit_sec=Inf)
    ...
    @timeit(
        ...
        solve_poi_no_params(
            data,() -> POI.Optimizer(GLPK.Optimizer()); vector_version=vector_version, params=params
        )
    )
end
...
function run_benchmark(;
    ...
    GC.gc()
    glpk_poi_no_params_scalar_obj = solve_poi_no_params_glpk(data, vector_version=false, time_limit_sec=time_limit_sec)
    @show glpk_poi_no_params_scalar_obj
    GC.gc()
    glpk_poi_moi_scalar_obj = solve_poi_glpk(data, vector_version=false, time_limit_sec=time_limit_sec)
    @show glpk_poi_moi_scalar_obj
    GC.gc()
    print_timer()
    println()
end
